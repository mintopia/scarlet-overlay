...
- **sailing/nav:** `speed_sog, speed_stw, vmg, heading_true, heading_magnetic, cog, magnetic_variation, depth_below_surface, depth_below_transducer, rate_of_turn, heel, pitch, trip_log` plus surfaced SignalK nav: `xte, bearing_to_wp_true, track_bearing_true, wp_distance, wp_ttg, current_set_true, current_drift, rudder_angle, autopilot_state`. **Use the actual live VM names confirmed 2026-06-19** (see `reference_signalk_unmapped_catalog`): the richer cluster is `scarlet_signalk_navigation_course_calcValues_*` (bearings/distance/ETA/timeToGo/VMG), `..._courseGreatCircle_*` (XTE = `_crossTrackError`, next/previous/following point lat/lon), `..._gnss_*`, `_position_latitude/longitude`, `_rateOfTurn`, `_attitude_yaw`, `_steering_autopilot_state`, `_steering_rudderAngle`, `_environment_current_setTrue`/`_drift`, `_depth_belowTransducer`. **NOTE: no normalized `source`-labelled series exist in prod yet — map these raw names now; the later normalization swap is upstream-dependent.**
...
- **weather/marine:** `wx_air_temp, wx_wind_speed, wx_wind_gust, wx_wind_dir, wx_pressure, sea_wave_height, sea_wave_period, sea_wave_direction, sea_current_speed, sea_current_dir` → mapped to the **already-live** `scarlet_weather_*` series (confirmed in VM 2026-06-19, `job="weather"`): `scarlet_weather_temperature_celsius`, `_wind_speed_kn`, `_wind_gusts_kn`, `_wind_direction_deg`, `_pressure_hpa`, `_wave_height_m`, `_wave_period_s`, `_wave_direction_deg`, `_sea_temperature_celsius`, `_current_speed_kn`, `_current_direction_deg`. (Emission already works; Phase 3 only adds scheduled freshness.)
...
`WeatherService` already fetches OpenMeteo forecast + marine (wave height/period/direction). `WeatherMetricsController` already exposes `scarlet_weather_*` for OTEL scrape — and as of 2026-06-19 these series **are live in VM** (`job="weather"`, see the Task 2.1 name list), so emission already works. The remaining gap is **freshness**: the fetch is on-demand (5-min cache) and unscheduled, so between scrapes the data can be stale. Add a scheduled command that warms the fetch every 15 min so the scrape always sees fresh data. Confirm the emitted names match exactly what the Task 2.1 weather/marine sources map.
