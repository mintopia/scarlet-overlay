...
class CanonicalCatalogController extends Controller
{
    ...
    private function validateMetric(Request $request): array
    {
        return $request->validate([
            ...
            'gate_metric_name' => ['nullable', 'string'],
            'gate_max_value' => ['nullable', 'numeric'],
            'gate_label_matchers' => ['nullable', 'array'],
            'gate_label_matchers.*.label' => ['required', 'string'],
            'gate_label_matchers.*.op' => ['required', 'in:equals,absent'],
            'gate_label_matchers.*.value' => ['nullable', 'string'],
            'enabled' => ['boolean'],
            ...
            'sources.*.label_matchers.*.label' => ['required', 'string'],
            'sources.*.label_matchers.*.op' => ['required', 'in:equals,absent'],
            'sources.*.label_matchers.*.value' => ['nullable', 'string'],
            ...
            'sources.*.unit_transform.*.op' => ['required', 'in:multiply,divide,subtract,add'],
            'sources.*.unit_transform.*.value' => ['required', 'numeric'],
