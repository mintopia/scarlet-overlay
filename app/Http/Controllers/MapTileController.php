...
class MapTileController extends Controller
{
    ...
    public function seamarkOverlay(OpenSeaMapService $mapService, int $z, int $x, int $y): StreamedResponse
    {
        $filename = $mapService->getSeamarkOverlay($z, $x, $y);
        if ($filename === null) {
            abort(404);
        }
    ...
    public function osm(OpenSeaMapService $mapService, int $z, int $x, int $y): StreamedResponse
    {
        $filename = $mapService->getPlainTile($z, $x, $y);
        if ($filename === null) {
            abort(404);
        }
    ...
    public function cartodbDark(OpenSeaMapService $mapService, int $z, int $x, int $y): StreamedResponse
    {
        $filename = $mapService->getPlainDarkTile($z, $x, $y);
        if ($filename === null) {
            abort(404);
        }
    ...
    public function depthContours(OpenSeaMapService $mapService, int $z, int $x, int $y): StreamedResponse
    {
        $filename = $mapService->getDepthContourTile($z, $x, $y);
        if ($filename === null) {
            abort(404);
        }
